...
export function getContrastWarnings(palette) {
  ...
  if (hasLowContrast(palette.foreground, palette.background)) {
    addWarning(warnings, 'textOnBackground', 'Text may be hard to read on the background.');
  }

  if (hasLowContrast(palette.headings, palette.background)) {
    addWarning(warnings, 'headingsOnBackground', 'Headings may be hard to read on the background.');
  }

  if (hasLowContrast(palette.link, palette.background)) {
    addWarning(warnings, 'linksOnBackground', 'Links may be hard to read on the background.');
  }
  ...
  if (hasLowContrast(palette.hover, palette.background)) {
    addWarning(warnings, 'hoverOnBackground', 'Hover color may be hard to see on the background.');
  }

  if (hasLowContrast(palette.tag, palette.background)) {
    addWarning(warnings, 'tagsOnBackground', 'Tags may be hard to read on the background.');
  }
  ...
  if (hasLowContrast(palette.hover, palette.link) && areAccentColorsConfusable(palette.hover, palette.link)) {
    addWarning(warnings, 'hoverVsLink', 'Hover color is hard to distinguish from link color.');
  }

  if (hasSameColor(palette.link, palette.linkVisited)) {
    addWarning(warnings, 'linkSameAsVisited', 'Link and visited link use the same color.');
  }

  if (hasSameColor(palette.link, palette.hover)) {
    addWarning(warnings, 'linkSameAsHover', 'Link and hover use the same color.');
  }

  if (hasSameColor(palette.linkVisited, palette.hover)) {
    addWarning(warnings, 'visitedSameAsHover', 'Visited link and hover use the same color.');
  }
